...
Fast jede Statistiksoftware verfügt über mehr oder weniger intuitive Werkzeuge zur Datenaufbereitung. In {{< fa-brands:r-project >}} sind insbesondere die Pakete `dplyr` und `tidyr` sehr hilfreich. Hier ein Beispiel für das Wrangling von "Wide" in "Long" Format:
```{r}
library(haven)
library(dplyr)
library(tidyr)
